...
so parity there is mostly CLI surfacing, not protocol work:
...
```bash
make build   # → bin/idev
make qa      # lint (gofmt/vet/tidy/staticcheck/govulncheck) + 6-combo cross-compile
```
...
- [blessed0x/xkvm-ios-injector](https://github.com/blessed0x/xkvm-ios-injector) — idev's device subsystem was battle-tested there first
